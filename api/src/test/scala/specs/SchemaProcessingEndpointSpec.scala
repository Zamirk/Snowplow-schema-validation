...
import snowplow.models.HttpResponces
import snowplow.service.SchemaProcessingEndpoint
import specs.Helpers.buildUri
    ...
    "with a POST request " - {
      ...
      "with a valid schema should return json response" in {
        ...
        generateRequest(request, f.endpoint).as[String].unsafeRunSync() shouldBe
          Json
            .toJson(
              HttpResponces.SchemaResult("uploadSchema", f.data_1.id, "success")
            )
            .toString()
